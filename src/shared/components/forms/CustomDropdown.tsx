...
import { Dropdown, getId, IDropdown, IDropdownProps, ISelectableDroppableTextProps, Label } from '@fluentui/react';
...
> = (
  props: ICustomDropdownProps &IDropdownProps & FormFieldWithValidationInterface
) => {
  const currentLang = useSelector((state: RootState) => state.reduxLanguage.language);
  const {
    className,
    readOnly,
    containerClassName,
  ...
  return (
    <div className={clsx('mol-vertialFlexDiv', containerClassName)}>
      ...
      <Dropdown
        {...props}
        id={fieldId}
        calloutProps={{className: callOutWidth !== 'Fixed' ? 'applyContentFit' : ''}}
        className={useClassName}
        errorMessage={
          readOnly ? undefined : errorMessageFromProps ?? errorMessage
        }
        onRenderContainer={(p: ISelectableDroppableTextProps<IDropdown,HTMLDivElement>|undefined,df?: (dfp: ISelectableDroppableTextProps<IDropdown,HTMLDivElement>|undefined) => JSX.Element |null)=>{
            if (callOutWidth !== 'Fixed') {                      
                const dd = document.getElementById(fieldId);
                document.body.style.setProperty('--currentMolDropDownMinSize', `${dd?.clientWidth.toString()}px`);
            }
