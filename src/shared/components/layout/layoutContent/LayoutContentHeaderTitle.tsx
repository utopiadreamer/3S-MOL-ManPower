...
const LayoutContentHeaderTitleComponent: FC<LayoutContentHeaderTitleProps &PropsFromRedux> = (props: LayoutContentHeaderTitleProps & PropsFromRedux) => {

    const {t} = useTranslation(['menu']);
    const history = useNavigate();
    const location = useLocation();
    const { menuEntry, pageBreadCrumbItems, dispatchChangeBreadcrumb, maxDisplayedItems, overflowIndex } = props;
    const renderMultipleTitle = (titleParts: LayoutMenuItem[]) => {
        const items = titleParts.map<IBreadcrumbItem>((itemTitle, index) => ({
            ...
            onClick: itemTitle.pathname !== '' && (((location.search ?? '')!=='') || (location.pathname !== itemTitle.pathname)) ? ()=>{history(itemTitle.pathname); dispatchChangeBreadcrumb([]); } : undefined
        }));
        const iconName = titleParts.length > 0 ? titleParts[titleParts.length-1].iconName : '';
        return (
            <>
                {
                    iconName &&
...
export const LayoutContentHeaderTitle = connector(LayoutContentHeaderTitleComponent);